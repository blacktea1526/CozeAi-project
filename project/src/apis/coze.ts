    ...
    async chat(options: CozeChatOptions): Promise<CozeChatResult> {
      try {
        ...
        if (result.success) {
          ...
          result.getLastAnswer = () => result.messages.filter(message => message.role === "assistant" && message.type === "answer")[result.messages.filter(message => message.role === "assistant" && message.type === "answer").length - 1]!;
